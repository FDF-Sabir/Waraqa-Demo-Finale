...
export default function Declaration({
  ...
  const [data, setData] = useState<any>(null),
    ...
    [confirm, setConfirm] = useState<"close" | "validate" | null>(null),
    [showAll, setShowAll] = useState(false);
  const scope = draft ? "all" : "reviewed";
  async function load() {
    setData(await api(`/workspace/releve?month=${month}&scope=${scope}`));
  }
  useEffect(() => {
    setPicked([]);
    run(load);
    ...
    <>
      ...
      {data.cloture && (
        <div className="u-info">
          <b>Relevé clôturé</b> le {new Date(data.cloture.createdAt).toLocaleString("fr-FR")} par {data.cloture.author} · {data.cloture.ids.length} ligne(s) · TVA {money(data.cloture.totaux.tva)} MAD.
          {admin && <button className="secondary small" onClick={() => run(async () => setData(await api("/workspace/releve/reopen", "POST", { month })), "Relevé rouvert")}>Rouvrir</button>}
        </div>
      )}
