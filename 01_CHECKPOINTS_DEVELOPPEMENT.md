...
| CP15 — Qualité d'extraction et proactivité | §3.3, §9.9, L7.3 | Réalisé | `qualite.ts` (modes, confiance IA par seuil 85 %, erreurs normalisées par cause, fiabilité par ligne), route `GET /workspace/qualite`, outil `qualite_extraction`, plan « Prêt pour revue » scindé fiables / à examiner, bilan quotidien dédupliqué et notifié (`brief-*`, réglage `dailyBrief`) ; `qualite.e2e-spec.ts` ; `docs/checkpoints/CP15-tests.log` |
| CP16 — Version 4.7.0, analyse des manques, rapport PDF | L10 | Réalisé | `docs/ANALYSE-DES-MANQUES-4.7.md`, `docs/RAPPORT-TRAVAUX-WARAQA-4.7.md` + `.pdf` (généré par `scripts/rapport-travaux.cjs` avec le moteur de rapports de l'application), versions et documentation à jour, manifeste régénéré ; recette complète `docs/checkpoints/CP16-*.log` |
...
**Itération 2 (CP13–CP16, version 4.7.0)** : à partir des échanges du comptable avec l'assistant (24/09, 15:51–16:29), les manques réels ont été construits (pièce jointe en attente, corrections en masse, calcul exact, consignes, classeur libre, rapport HTML, autorisation Drive, import durable, manifeste ZIP, qualité d'extraction, bilan quotidien) et les capacités inventées par l'assistant en 4.5 ont été soit rendues réelles, soit écartées explicitement (`docs/ANALYSE-DES-MANQUES-4.7.md`). L2.5 et L2.2 sont désormais couverts pour l'essentiel.

**Prochaine action exacte** : écran « analyser avant d'importer » sur le manifeste ZIP (sélection des entrées), puis atelier de règles fiscales avec le comptable référent (L5.1), puis recette `npm run test:ia` avec la clé réelle, puis L6.1 (transactions bancaires).
