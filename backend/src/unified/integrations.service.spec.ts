...
describe('Contrats des connecteurs, aucun service réel',()=>{
 ...
 beforeEach(()=>{
  ...
  process.env.GOOGLE_CLIENT_ID='fake-client';process.env.GOOGLE_CLIENT_SECRET='fake-test-secret';process.env.GOOGLE_REDIRECT_URI='http://localhost:3000/api/workspace/drive/callback';process.env.WARAQA_JWT_SECRET='test-encryption-only';
  mock=jest.spyOn(global,'fetch').mockRejectedValue(new Error('No network in tests'));
 });
 afterEach(()=>{mock.mockRestore();delete process.env.GOOGLE_CLIENT_ID;delete process.env.GOOGLE_CLIENT_SECRET;delete process.env.GOOGLE_REDIRECT_URI;delete process.env.WARAQA_JWT_SECRET;});
 it('consentement avec état unique, PKCE et portée limitée',async()=>{
  const start=await service.start({sub:1});const url=new URL(start.url);expect(url.searchParams.get('scope')).toBe('https://www.googleapis.com/auth/drive.file');expect(url.searchParams.get('code_challenge_method')).toBe('S256');expect(mock).not.toHaveBeenCalled();
  mock.mockResolvedValue(new Response(JSON.stringify({access_token:'fake-access',refresh_token:'fake-refresh',expires_in:3600}),{status:200}));
