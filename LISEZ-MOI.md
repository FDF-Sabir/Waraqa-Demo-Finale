...
- **Agent comptable IA** (Claude Sonnet 5) qui **fait le travail** : joignez le dossier du mois (ZIP, PDF, photos, Excel, CSV, JSON) ou collez le lien d’un dossier Drive, et demandez en langage courant (« traite tout juillet, rapproche les paiements, livre-moi le relevé en XML SIMPL et en Excel DGI et un tableau par fournisseur »). L’agent importe (et reprend seul la demande à la fin de l’import), lit, corrige les lignes mal lues d’après les pièces, rattache les déductions tardives, rapproche les paiements, confirme les désignations, crée les snapshots et **livre les fichiers en téléchargement direct** : Excel, PDF, CSV, Sage, JSON, XML SIMPL, Excel DGI, snapshot, sauvegarde, et **tableaux sur mesure** (période, fournisseur, taux, désignation, statut, colonnes, regroupement). Tout est tracé « Agent IA (pour …) » avec valeurs avant/après. **Deux validations restent au comptable**, en un clic chacune : marquer les lignes revues (attestation du contrôle des pièces) et clôturer le mois (archivage aussi). Détail : `docs/IA-ASSISTANT.md`.
